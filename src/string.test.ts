...
describe('stringify', () => {
  ...
  describe('round trip', () => {
    it.each([
      ...
      [{ '-webkit-transform': 'scale(1)', transform: 'scale(1)' }],
    ])('objectify(stringify(%s)) returns the original object', (object) => {
      expect(objectify(stringify(object))).toEqual(object);
    });

    it('round trips camelCase objects through kebabCase', () => {
      const object = { fontSize: '16px', backgroundColor: '#fff' };
      const css = stringify(object, { kebabCase: true });

      expect(css).toBe('font-size:16px;background-color:#fff');
      expect(objectify(css, { camelCase: true })).toEqual(object);
    });
  });
});
